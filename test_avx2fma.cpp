...
TEST(TFCpp, Avx2Fma)
{
    __m256 a[length], b[length], c[length];
    ...
    double result;
    {
        ...
        for ( unsigned int i = 0; i < iterations; ++i ) {
            for ( unsigned int j = 0; j < length; ++j ) {
                c[j] = _mm256_fmadd_ps(a[j], b[j], c[j]);
            }
        }
    }
    ...
    cout << "time per multiply-accumulate operation using AVX2: " << nanoseconds_per_fma_operation << " nanoseconds\n";

    cout << "time for " << numFmaOps << " multiply-accumulate operations using AVX2: " << result << " seconds\n";
}
